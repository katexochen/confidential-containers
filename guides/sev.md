...
The CoCo project has created a sample encrypted container image ([encrypted-image-tests](https://github.com/users/fitzthum/packages/container/encrypted-image-tests/61923658?tag=encrypted)). This image is encrypted using a key that comes already provisioned inside the `simple-kbs` for ease of testing. No `simple-kbs` policy is required to get things running.
